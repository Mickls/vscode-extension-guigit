...
(function () {
    ...
    window.addEventListener('message', event => {
        ...
        switch (message.type) {
            ...
            case 'commitHistory':
                ...
                break;
            case 'smartRefresh':
                // 智能刷新：保持当前筛选状态，重新加载数据
                console.log('Smart refresh triggered, preserving current filters');
                handleSmartRefresh();
                break;
            case 'newCommits':
                // 处理新提交数据，进行增量更新
                console.log('Received new commits for incremental update');
                handleNewCommits(message.data);
                break;
    ...
    /**
     * 处理智能刷新
     * 保持当前筛选状态，只更新仓库和分支信息，并进行增量提交更新
     */
    function handleSmartRefresh() {
        console.log('Handling smart refresh - preserving filter state');
        
        // 保存当前的筛选状态
        const currentBranch = getState('currentBranch');
        ...
        console.log('Preserved state:', {
            ...
            selectedCommitsCount: selectedCommits ? selectedCommits.length : 0,
            currentCommitsCount: currentCommits ? currentCommits.length : 0
        });
        ...
        // 进行增量提交更新
        performIncrementalCommitUpdate();
    }
     ...
     * 只获取最新的提交，避免重新加载所有数据
     */
    function performIncrementalCommitUpdate() {
        const currentCommits = getState('commits');
        const currentBranch = getState('currentBranch');
        const authorFilter = getState('authorFilter');
        ...
        if (!currentCommits || currentCommits.length === 0) {
            console.log('No existing commits, performing full load');
            loadCommits(true);
            return;
        }
        
        // 获取当前最新提交的哈希值作为参考点
        const latestCommitHash = currentCommits[0].hash;
        
        console.log('Checking for new commits since:', latestCommitHash.substring(0, 8));
        ...
        vscode.postMessage({
            ...
            sinceCommit: latestCommitHash,
            limit: 50 // 限制一次最多获取50个新提交
        });
    }

    /**
     * 处理新提交数据
     * @param {Object} data - 包含新提交数据的对象
     * @param {Array} data.newCommits - 新的提交记录数组
     * @param {string} data.sinceCommit - 参考提交哈希
     */
    function handleNewCommits(data) {
        const { commits: newCommits, sinceCommit, hasMore } = data;
        ...
        console.log(`Found ${newCommits.length} new commits since ${sinceCommit.substring(0, 8)}`);
        
        const currentCommits = getState('commits');
        const currentCommit = getState('currentCommit');
        
        // 将新提交添加到现有提交列表的开头
        const updatedCommits = [...newCommits, ...currentCommits];
        setState('commits', updatedCommits);
        
        // 增量更新UI，在列表顶部插入新提交
        insertNewCommitsAtTop(newCommits);
        
        // 更新总提交数量
        const totalCommits = getState('totalCommits');
        setState('totalCommits', totalCommits + newCommits.length);
        setState('loadedCommits', updatedCommits.length);
        ...
        console.log(`Updated commit list: ${updatedCommits.length} total commits`);
    }
     ...
     */
    function insertNewCommitsAtTop(newCommits) {
        const panelHeader = commitList.querySelector('.panel-header');
        const firstCommitElement = commitList.querySelector('.commit-item');
        
        // 为新提交创建元素并添加动画
        newCommits.forEach((commit, index) => {
            const commitElement = createCommitElement(commit, index);
            commitElement.classList.add('new-commit', 'entering');
            
            // 插入到第一个提交元素之前，或者在panel-header之后
            if (firstCommitElement) {
                commitList.insertBefore(commitElement, firstCommitElement);
            } else if (panelHeader) {
                commitList.insertBefore(commitElement, panelHeader.nextSibling);
            } else {
            ...
            requestAnimationFrame(() => {
                ...
                setTimeout(() => {
                    commitElement.classList.remove('new-commit', 'entering', 'animate');
                }, 300);
            });
        });
